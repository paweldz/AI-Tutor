...
import { readJSON, writeJSON, sKey } from "./storage.js";

export const XP_KEYS = { xp: "gcse_xp_v1", streaks: "gcse_streaks_v1" };

export function todayStr() { return new Date().toISOString().slice(0, 10); }

export function loadXP() { return readJSON(sKey(XP_KEYS.xp), { total: 0, history: [] }); }
export function saveXP(data) { writeJSON(sKey(XP_KEYS.xp), data); }
...
export function loadStreaks() { return readJSON(sKey(XP_KEYS.streaks), { dates: [] }); }
export function saveStreaks(data) { writeJSON(sKey(XP_KEYS.streaks), data); }
